...
struct io_wqe_acct {
	 ...
	 * For 2), we use rwsem since nr_fixed, max_works, fixed_workers
	 * are barely written but frequently read when inserting work
	 */
	union {
		raw_spinlock_t lock;
...
static int io_wqe_worker(void *data)
{
	...
	if (test_bit(IO_WQ_BIT_EXIT, &wq->state) ||
	    (fixed && (worker->flags & IO_WORKER_F_EXIT))) {
		if (fixed)
			io_worker_handle_private_work(worker);
		else
			io_worker_handle_public_work(worker);
...
static void io_wqe_enqueue(struct io_wqe *wqe, struct io_wq_work *work)
{
	...
	if (wqe->fixed_worker_registered && !io_wq_is_hashed(work)) {
		if (io_wqe_insert_private_work(wqe, work,
					io_get_acct(wqe, !acct->index, true)))
			return;
	}
	raw_spin_lock(&acct->lock);
			...
			struct io_uring_fixed_worker_arg *new_count)
{
	...
	for_each_node(node) {
		...
		first_node = false;
		wqe->fixed_worker_registered = true;
	}
	rcu_read_unlock();
